...
overview: "NT-first 0DTE options extension layer. Primary goal: trade crypto 0DTE options (Deribit first), IB equity 0DTE second. Phases 1–8 delivered: venue-agnostic core, Deribit path, costs/attribution, multi-strategy selector + approval, IB secondary venue. Phase 9 next: optional streaming capture + research."
todos:
  - id: scaffold-package
  ...
  - id: phase9-optional
    ...
    status: completed
  - id: phase9plus-config-wiring
    content: Audit and reduce hardcoded venue/selector/cost defaults — wire IB/Deribit/reference paths fully through layered YAML; align schema defaults with venue; optional fixture-from-profile generation
    status: completed
isProject: false
---
...
Design v2 in [`docs/design/`](docs/design/) is complete. **Phases 1–8 are delivered** — venue-agnostic gates, actors, FSM, journal, reference strategy plumbing, venue adapter registry, Deribit combo spread path, cost-aware attribution, multi-strategy SelectorActor with approval routing, and IB equity BAG spread secondary path on `BacktestNode` / `TradingNode`.
...
**Vertical slice proof:** `configs/profiles/backtest_spy.yaml` + extended SPY catalog → JSONL: gates → BAG spread `ORDER_SUBMIT` → `FILL` → `PNL` + `LEARNING_RECORD` with commission. Manual: `paper_spy.yaml` + IB paper account (operator).
...
| --- | --- | --- |
| **Config wiring cleanup** | Before adding venues or operator tuning pain | Audit hardcoded defaults across Phases 1–8; single source of truth via YAML — see **9.2** |
...
### 9.2 Config wiring cleanup (hardcoded defaults audit)

**Goal:** Reduce duplicated and venue-assumptive hardcoded values introduced across Phases 1–8 vertical slices. YAML overlays remain the operator source of truth; code defaults are fallbacks only — not silent overrides.

**Trigger:** Phase 9+ or whenever adding a new venue/profile causes copy-paste defaults in selectors, reference strategy, or fixture scripts.

**Scope (audit targets):**

| Area | Current pain | Target |
| --- | --- | --- |
| `config/schema.py` | Crypto-first defaults (`settlement_currency: BTC`, `08:00` expiry) on IB-default schema | Venue-neutral or adapter-aware defaults; profiles stay explicit |
| `strategies/selectors/` | IB: `venue`, `market_close_utc`, `multiplier`, fee fallback in `IbStructureSelector`; Deribit: `BTC`/`08:00` defaults | `registry.py` passes `VenueConfig`, `SessionConfig`, `ReferenceStrategyConfig`, `FeeScheduleConfig` — no duplicate fee schedules in selectors |
| `strategies/reference.py` | `"BTC"` / `"SPY"` fallbacks when `option_series_id` missing | Require profile fields or derive from `underlying`; pass `venue.name` into series id builders |
| `costs/` | `multiplier=100.0` hardcoded for IB edge math | Instrument-derived or `reference.option_multiplier` config field |
| `scripts/build_*_catalog_fixture.py` | Constants duplicated from profile YAML (`expiry`, strikes, venue) | Optional: read profile YAML or shared `tests/fixtures/catalog/manifest.yaml` so fixture ↔ profile cannot drift |
| Unit tests | Fixed numeric fixtures (`400`, `0.65`) | Keep as pure-function inputs — document that they mirror `configs/fees/*.yaml` and catalog manifest |

**Work items:**

| Work item | Module / path | Notes |
| --- | --- | --- |
| ADR | `docs/implementation/config-wiring.md` | Document single-source-of-truth rule: YAML → factory → strategy/selector; when code defaults are allowed |
| Schema pass | `config/schema.py`, `ReferenceStrategyConfig` | Add missing IB fields (`option_multiplier`, `option_venue`?) if needed; remove misleading crypto defaults on IB path |
| Registry wiring | `strategies/selectors/registry.py`, `node/factory.py` | Thread venue, session close, multiplier, fees into both Deribit and IB selectors |
| Reference strategy | `strategies/reference.py` | Remove `"BTC"`/`"SPY"` string fallbacks; fail closed on missing `option_series_id` when selector enabled |
| Fixture alignment | `scripts/build_catalog_fixture.py`, `build_deribit_catalog_fixture.py` | Shared manifest or profile-driven generation to prevent catalog/profile drift |
| Tests | `tests/unit/test_config.py`, selector tests | Assert loaded profiles supply values that reach selectors (no silent defaults in integration path) |

**Non-goals:**

- Do not remove all Pydantic defaults — partial YAML loads and tests still need safe fallbacks.
- Do not drive runtime behavior from committed catalog fixture constants in production code.

**Vertical slice proof:** Load `backtest_spy.yaml` / `backtest_btc.yaml` → assert selector receives venue, session close, and fee schedule from config (unit or integration probe); zero duplicate fee constants in selector `__init__` fallbacks.
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| **Coder** | Gates, FSM, reference plumbing | Venue adapter registry, crypto config | Deribit spread selector, catalog fixture | `FeeModel`, `LearningModule`, edge cost math ✓ | Selector, approval ✓ | IB selector + fees ✓ | `StreamingFeatherWriter`, config wiring cleanup |
...
| Unit | IB structure selector, IB fee model wiring, IB edge cost math | Phase 8 ✓ |
| Integration | SPY catalog IB BAG spread → fill → PnL + `LEARNING_RECORD` + commission | Phase 8 ✓ |
...
| **8** ✓ | IB BAG spread path on `backtest_spy.yaml`; `FixedFeeModel` for IB backtest; IB fee-aligned edge gate; **80 tests green** |
| **9** | Optional: `StreamingFeatherWriter` capture → `catalog convert` → backtest replay; config wiring cleanup (9.2) removes duplicated selector/schema/fixture defaults |

---

## Key files — next up (Phase 9 order)

1. (Recommended early) **9.2 Config wiring cleanup** — `docs/implementation/config-wiring.md`; wire selectors/reference through `AppConfig`; fixture ↔ profile alignment
2. [`src/nautilus_zerodte/config/schema.py`](src/nautilus_zerodte/config/schema.py) — `StreamingConfig` model
2. [`configs/streaming/default.yaml`](configs/streaming/default.yaml) — feather paths, include_types, rotation
3. [`src/nautilus_zerodte/node/streaming.py`](src/nautilus_zerodte/node/streaming.py) — `StreamingFeatherWriter` attach on `build_trading_node()`
4. [`src/nautilus_zerodte/cli/main.py`](src/nautilus_zerodte/cli/main.py) — `catalog convert` command
5. [`docs/implementation/live-catalog-capture.md`](docs/implementation/live-catalog-capture.md) — Feather vs JSONL journal ADR
6. Operator data dirs — `data/streaming/`, `data/catalogs/` (gitignored)

## Key files delivered (Phase 8)

1. [`src/nautilus_zerodte/strategies/selectors/ib.py`](src/nautilus_zerodte/strategies/selectors/ib.py) — IB BAG spread selector; catalog-friendly spread id
2. [`src/nautilus_zerodte/costs/ib.py`](src/nautilus_zerodte/costs/ib.py) — `ib_edge_after_cost_bps`, per-contract commission bps
3. [`configs/fees/ib_options.yaml`](configs/fees/ib_options.yaml) — per-contract `FixedFeeModel` schedule
4. [`configs/session/us_equity.yaml`](configs/session/us_equity.yaml) — T-30m blackout before US equity close
5. [`configs/profiles/paper_spy.yaml`](configs/profiles/paper_spy.yaml) — IB secondary operator profile (`dry_run: true`)
6. [`configs/profiles/backtest_spy.yaml`](configs/profiles/backtest_spy.yaml) — IB BAG spread backtest profile
7. [`scripts/build_catalog_fixture.py`](scripts/build_catalog_fixture.py) — extended SPY catalog with options, spread, greeks
8. [`src/nautilus_zerodte/node/factory.py`](src/nautilus_zerodte/node/factory.py) — `FixedFeeModel` for IB; option-chain data config for IB + Deribit
9. [`src/nautilus_zerodte/config/loader.py`](src/nautilus_zerodte/config/loader.py) — auto-merge `fees/ib_options.yaml` for IB profiles
10. Tests — [`tests/unit/test_ib_selector.py`](tests/unit/test_ib_selector.py), [`tests/unit/test_ib_costs.py`](tests/unit/test_ib_costs.py), [`tests/integration/test_ib_backtest.py`](tests/integration/test_ib_backtest.py); **80 tests green**
...
## Phase 8 handoff (completed 2026-06-26)

**Vertical slice proof:** `configs/profiles/backtest_spy.yaml` + `tests/fixtures/catalog` → JSONL contains `GREEK_PASSED` → `ORDER_SUBMIT` (SPY-CS spread) → `FILL` with `commission` → `PNL` → `LEARNING_RECORD`.

### Delivered

| Area | Path | Notes |
| --- | --- | --- |
| IB selector | `strategies/selectors/ib.py`, `selectors/registry.py` | ATM ± N call vertical; catalog-friendly BAG symbol `SPY-CS-{expiry}-{K1}_{K2}.NYSE` |
| IB costs | `costs/ib.py` | Per-contract commission bps; `ib_edge_after_cost_bps` aligned to fee schedule |
| Fee config | `configs/fees/ib_options.yaml` | $0.65/contract × 2 legs; auto-loaded for IB profiles |
| Schema + loader | `config/schema.py`, `config/loader.py` | `commission_per_contract`, `contracts_per_spread`; IB fee overlay merge |
| Factory | `node/factory.py` | `FixedFeeModel` on IB backtest (skipped when `backtest_plumbing`); option-chain data for IB + Deribit reference |
| Reference strategy | `strategies/reference.py` | IB `subscribe_option_chain` via `ib_option_series_id` |
| Session | `configs/session/us_equity.yaml` | `us_equity_close` expiry mode; WARM 60s chain interval |
| Profiles | `configs/profiles/paper_spy.yaml`, `backtest_spy.yaml` | IB secondary operator + backtest paths |
| Catalog | `scripts/build_catalog_fixture.py`, `tests/fixtures/catalog/` | SPY equity + 0DTE calls + spread + greeks parquet slice |
| Tests | `test_ib_selector.py`, `test_ib_costs.py`, `test_ib_backtest.py` | **80 tests green** |

### Known constraints carried into Phase 9

1. **IB spread id is catalog-simplified** — `SPY-CS-…` format avoids parentheses in parquet SQL; live IB BAG via `new_generic_spread_id()` deferred until operator paper validates NT IB spread loading.
2. **Option contracts use NYSE venue in fixture** — aligns with `BacktestVenueConfig.name`; live IB may route via SMART/OPRA.
3. **`backtest_plumbing: true`** remains for `backtest_reference.yaml` legacy regression — no `FixedFeeModel` on plumbing path.
4. **`JournalActor` refactor deferred** — injected `Journal` per strategy + selector is sufficient.
5. **Human approval is a stub** — auto-approves with audit journal.
6. **Dual journal paths unchanged** — factory `NODE_*` + strategy/selector journals append to same JSONL path.
7. **Deribit regression trails unchanged** — `backtest_btc.yaml`, `backtest_btc_multi.yaml` remain green.
8. **Hardcoded defaults audit deferred** — IB/Deribit selector venue, session, multiplier, and schema crypto defaults not fully config-wired; see Phase **9.2**.

### Phase 9 start checklist

1. (Optional, recommended early) **Config wiring cleanup (9.2)** — audit hardcoded defaults from Phases 1–8; wire selectors/reference through `AppConfig`; align fixture scripts with profiles.
2. Add `StreamingConfig` to `config/schema.py` — `enabled` (default `false`), paths, `include_types`, rotation.
3. Add `configs/streaming/default.yaml` overlay — align `include_types` with ingestion tiers (HOT/WARM only).
4. Implement `node/streaming.py` — attach `StreamingFeatherWriter` on `build_trading_node()` when enabled.
5. Add `catalog convert` CLI — `ParquetDataCatalog.convert_stream_to_data()` with optional time-range trim.
6. Add `--streaming` flag to `paper` command; gitignore `data/streaming/`, `data/catalogs/`.
7. Write `docs/implementation/live-catalog-capture.md` — Feather vs JSONL journal; two-track catalog strategy.
8. **Regression:** all Phase 8 trails (`backtest_spy`, `backtest_btc`, `backtest_reference`) remain green.
9. **Vertical slice proof for Phase 9:** `paper --streaming` on Deribit testnet → stop → `catalog convert` → `backtest` replays with journal trail.
